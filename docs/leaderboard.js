...
const state = {
  ...
  filtered: [],
  sortKey: "score",
  sortDir: "asc", // MAE: Lower is better, so default to Ascending
  hiddenCols: new Set(),
};
  ...
  document.querySelectorAll("#tbl thead th").forEach(th => {
    th.addEventListener("click", () => {
      ...
      if (state.sortKey === k) {
        ...
        state.sortKey = k;
        state.sortDir = (k === "score") ? "asc" : "desc";
      }
      applyFilters();
    });
  });
...
async function main() {
  ...
  try {
    // Corrected path to point to your data folder
    const res = await fetch("../leaderboard/leaderboard.csv", { cache: "no-store" });
    const txt = await res.text();
    const rows = parseCSV(txt);

    // MAPPING BOT DATA TO UI DATA
    const cleaned = rows
      .filter(r => r.team) 
      .map(r => ({
        timestamp_utc: r.date, // Bot uses 'date'
        team: r.team,          // Bot uses 'team'
        model: "GNN",          // Placeholder
        score: r.mae,           // Bot uses 'mae'
    ...
    // Default to sorting by MAE ascending
    state.sortKey = "score";
    state.sortDir = "asc";
    applyFilters();
  } catch (e) {
